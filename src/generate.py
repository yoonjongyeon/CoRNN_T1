...
if __name__ == '__main__':
    ...
    cnn.load_state_dict(torch.load(os.path.join(MODEL_DIR, 't1_cnn.pt'), map_location=torch.device('cpu')))
    rnn.load_state_dict(torch.load(os.path.join(MODEL_DIR, 't1_rnn.pt'), map_location=torch.device('cpu')))
